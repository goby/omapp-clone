...
function setday(tt,obj) //主调函数
{
	...
	outButton = (arguments.length == 1) ? null : th;	//设定外部点击的按钮	if(th.type=="text")	{        outOtherObject=(arguments.length == 1) ? null : obj;//其它输入框
...
function WriteDateTo(yy,mm,n,hh,mi)
{
	if (bUseTime)
	{
		...
		if(outOtherObject != null)
		{
		    outObject.value="";
		    var year =Number(yy) +20;
		    outObject.value= yy+ "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + "00"; //注：在这里你可以输出改成你想要的格式
		   
		    outOtherObject.value= year + "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + "00"; //注：在这里你可以输出改成你想要的格式
	...
	else
	{
		...
		if(outOtherObject != null)
		{
		   ...
		   outOtherObject.value= year + "-" + format(mm) + "-" + format(n); //注：在这里你可以输出改成你想要的格式
